...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
	...
	var $ = layui.$,
		...
		TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
			...
			, '<div class="layui-table-box">', '{{# if(d.loading){ }}', '<div class="layui-table-init" style="background-color: #fff;">', '<i class="layui-icon layui-icon-loading layui-icon"></i>', '</div>', '{{# } }}'
	...
	Class.prototype.renderToolbar = function () {
		...
		// EOVA 003 自定义工具栏
		layui.each(options.diyToolbar, function (i, o) {
			iconElem.push('<div class="layui-inline" title="' + o.title + '" lay-event="' + o.layEvent + '">' +
				'<i class="layui-icon ' + o.icon + '"></i>' +
				'</div>');
		});
	...
	Class.prototype.pullData = function (curr) {
		...
		if (options.url) { // Ajax请求
			...
			// EOVA 001 首次不加载数据
			if (!that.config.isFirstLoad) {
				// 第二次开始加载数据
				that.config.isFirstLoad = true;
				return;
			}
				...
				layui.each(data, function (i1, item1) {
					...
					that.eachCols(function (i3, item3) {
						...
						var td = ['<td data-field="' + field + '" data-key="' + key + '" ' + function () { // 追加各种属性
							...
							//if (item3.templet) attr.push('data-content="' + content + '"'); // 自定义模板 EOVA 临时去掉 JS内容会导致异常
							...
							return item3.templet ? function () {
								return typeof item3.templet === 'function' ?
									// EOVA 005 格式化函数回调 参数保持和EasyUI 一致 内容,行,列索引,字段名
									item3.templet(content, tplData, i3, field) :
	...
	Class.prototype.fullSize = function () {
		...
		if (height == '') {
			var $ele = that.elem;
			var ph = $ele.parent().height(); // parent
			var mh = $ele.outerHeight(true) - $ele.outerHeight(); // margin
			height = ph - mh;
				...
				case 'LAYTABLE_EXPORT': // 导出
					if (device.ie) {
						...
						openPanel({
							list: function () {
								return [
									'<li data-type="xls">导出到 Csv 文件</li>', '<li data-type="xls">导出到 Excel 文件</li>'
								].join('')
							}(),
	...
	// EOVA 007 表格选中行索引值
	table.checkIndex = function (id) {
		var index = -1,
			data = table.cache[id] || [];
		// 计算全选个数
		layui.each(data, function (i, item) {
			if (item[table.config.checkName]) {
				index = item.LAY_TABLE_INDEX;
			}
		});
		return index;
	};
	...
	// EOVA 004 表格选中行 (单选)(行索引,从0开始)
	table.checkRow = function (id, index) {
		...
		var $tr = that.layBody.find('tr:eq(' + index + ')')
		var rd = $tr.find(':radio');
		if (rd.length > 0) {
			rd.prop('checked', !rd.is(':checked'));
			form.render('radio');
		}
		// 选中行
		that.setThisRowChecked(index);
	...
	// EOVA 004.1 表格选中行(可多选) (行索引,从0开始)
	table.checkRows = function (id, index) {
		var that = thisTable.that[id],
			thisData = table.cache[that.key],
			ELEM_CLICK = 'layui-table-click';
	...
	// EOVA 006 获取当前列DOM
	table.getHeader = function (id) {
		var that = thisTable.that[id],
			thisData = table.cache[that.key];

		return that.layHeader;
	};

	// EOVA 008 根据ID 获取实例对象
	table.getIns = function (id) {
